...
- управление фото в редактировании пока простое: новые файлы добавляются перед
  уже существующими превью;
- старые MinIO-файлы удаляются, если их убрали из списка;
- внешний URL не удаляется из storage, потому что он не принадлежит ReBloom.
