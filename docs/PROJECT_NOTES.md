...
Auction UI начат 2026-06-17:

- создание объявления с типом `auction` использует существующее поле `price` как
  начальную ставку;
- новые аукционы получают `expiresAt` через 2 часа вместо обычных 48 часов;
- карточка и модалка показывают начальную/текущую ставку, оставшееся время и
  optional `auctionUserBid`/`auctionUserBidStatus` для будущей bid-модели;
- полноценные ставки, история ставок и проверка победителя пока не реализованы,
  для них нужна отдельная схема БД и серверные permissions.
...
Security & performance hardening проведён 2026-07-05 (полный аудит + фиксы):

- Auth: удалён неиспользуемый `signUpAction` (позволял захватить аккаунт без
  подтверждения email); OAuth больше не привязывается к существующему аккаунту
  автоматически по совпадению email; подпись Google ID token проверяется через JWKS;
  единое сообщение «Неверный email или пароль» + выравнивание времени ответа;
  смена пароля отзывает остальные сессии; cookie всегда `Secure` в production;
  одновременно действителен только последний magic-link/email-код;
  на complete-регистрацию по коду действует общий rate limit с verify.
- S3/фото: ключи загрузок содержат userId (`listing-images/<userId>/...`),
  сервер принимает только собственные загрузки пользователя или https-ссылки;
  удаление ограничено whitelist-префиксами; presigned PUT подписывает Content-Type;
  image proxy форсирует Content-Type по расширению + `nosniff`;
  `profile-avatars/` добавлен в читаемые префиксы proxy (аватары снова отдаются).
- Целостность: ставки аукциона идут через `SELECT ... FOR UPDATE`; mark-sold —
  атомарный `updateMany` со статусом и обязательным `buyerId`; статусы «Продано/Снять»
  меняются атомарно; аукцион нельзя редактировать (сервер + скрыта кнопка);
  цены/количества — только целые числа; новый чат нельзя открыть по не-ACTIVE
  объявлению (кроме победителя аукциона).
- Performance: лимиты и `select` вместо полного `User` в чате и поддержке
  (нет утечки `passwordHash` в память процесса), `take` на диалоги/сообщения/мои
  объявления; `_count` вместо загрузки всех ставок; `/api/listings?city=` фильтрует
  на сервере, шелл рефетчит при смене города; главная грузит данные через
  `Promise.all`; next/image optimizer включён для известных хостов.
- Rate limit стал атомарным (advisory lock в транзакции); добавлены лимиты на
  избранное и ответы админа в поддержке; повторная жалоба на то же объявление —
  не чаще раза в 24 часа.
- Миграция `20260705000100_add_query_indexes`: составные индексы
  `(status, createdAt)`, `(status, soldAt)`, `(status, expiresAt)` на Listing и
  `updatedAt` на Conversation. Локально не применена (БД была выключена) — выполнить
  `npm run db:migrate`.

Yandex Metrica уточнена 2026-06-22:

- ecommerce выключен, потому что сделки проходят без корзины и оплаты на сайте;
- продажи букетов считаются через JS-цель `listing_marked_sold`;
- добавлены цели для `listing_viewed`, `auth_required`, `photo_upload_failed`,
  `listing_favorited`;
- `listing_favorited` пока не отправляется из UI: серверный action есть, но актуального
  пользовательского элемента избранного в интерфейсе не найдено.
...
- Отправка сообщений: `revalidatePath` для страницы чата и списка чатов на главной.
- `Listing.soldToBuyerId` — покупатель при «Продано» из чата; из «Мои объявления» без чата
  покупатель не заполняется.
- 2026-06-18: список чатов ревалидируется после отправки сообщения; карточки чатов ведут
  продавца в конкретный `conversationId`, чтобы несколько покупателей одного букета не
  схлопывались в последний диалог. Мобильные разделы синхронизируются с URL для корректного
  pull-to-refresh.
- 2026-06-22: админка получила создание витринных объявлений без AI:
  `/admin/listings/create` создаёт обычный `Listing` от demo-продавца с email
  `demo+slug@getflora.local`, фото грузятся в S3, статус можно выбрать `ACTIVE` или `SOLD`.
- `/admin/demo-chats` показывает только диалоги demo-продавцов и отправляет ответы с
  `senderId` demo-продавца, поэтому покупатель видит обычный ответ продавца; чаты реальных
  продавцов не доступны из этого раздела.
...
- Проданное объявление остается в `Мои объявления` 48 часов после `soldAt`.
...
- 2026-06-18: проданные объявления остаются в главной ленте и `Мои объявления` 48 часов
  после `soldAt`, показываются как нетапабельные карточки с красной плашкой `Продано`.
...
Миграция `20260621000100_add_auction_bids` добавляет:

- `AuctionBid` для абсолютных ставок по аукционам;
- связь ставок с `Listing` и `User`;
- индексы по `listingId`, `amount`, `createdAt`, `bidderId`.

Текущий auction-flow:

- у аукциона нет начальной ставки в UX;
- первая ставка может быть любой положительной суммой, следующая должна быть больше текущей;
- завершение аукциона определяется также на клиенте по `expiresAt`, чтобы проигравшие сразу видели blur и не могли открыть карточку;
- победитель видит `Вы выиграли` и может открыть чат с продавцом;
- desktop-кнопка `В чат` открывает общий раздел чатов с выбранным диалогом, mobile ведет в отдельный экран диалога.
