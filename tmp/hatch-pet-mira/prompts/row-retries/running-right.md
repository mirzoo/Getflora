Create Codex pet row `running-right` for `mira`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: compact friendly flower-coder mascot; thoughtful builder energy; small blossom cap, tiny satchel, tidy posture; inspired by Getflora resale marketplace and Russian-language engineering workflow; no logos, no text, no UI. Preserve silhouette, face, palette, material, proportions, markings, and props.
