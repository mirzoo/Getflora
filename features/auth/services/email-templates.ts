...
export function buildEmailCodeEmail({ code, ttlMinutes }: EmailCodeInput) {
  const safeCode = escapeHtml(code);
  const subject = "Код входа в Getflora";

  const text = [
    "Здравствуйте!",
    "",
    `Ваш код входа в Getflora: ${code}`,
    "",
    `Код действует ${ttlMinutes} минут и может быть использован один раз.`,
    "Если вы не запрашивали вход, просто проигнорируйте это письмо.",
    "",
    "-- Getflora",
    "https://getflora.ru",
...
<html lang="ru">
  ...
  <body style="margin:0;padding:0;background-color:#f6f7f8;color:#222222;font-family:Inter,Arial,Helvetica,sans-serif;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;">
      Одноразовый код для входа в Getflora. Действует ${ttlMinutes} минут.
    </div>
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#f6f7f8;padding:32px 16px;">
      <tr>
        <td align="center">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:520px;background-color:#ffffff;border:1px solid #d9dce1;border-radius:18px;overflow:hidden;box-shadow:0 16px 40px rgba(17,25,40,0.08);">
            <tr>
              <td style="padding:30px 28px 10px;text-align:center;">
                <div style="display:inline-block;font-size:24px;line-height:1;font-weight:800;color:#fd4604;">Getflora</div>
                <p style="margin:8px 0 0;font-size:13px;line-height:1.5;color:#8891a3;">Маркетплейс подаренных букетов</p>
              </td>
            </tr>
            <tr>
              <td style="padding:10px 28px 0;text-align:center;">
                <h1 style="margin:0;font-size:24px;line-height:1.25;color:#222222;">Код для входа</h1>
                <p style="margin:12px 0 0;font-size:15px;line-height:1.6;color:#8891a3;">
                  Введите этот код в окне входа. Он действует <strong style="color:#222222;">${ttlMinutes} минут</strong>.
                </p>
              </td>
            </tr>
            <tr>
              <td style="padding:24px 28px 10px;text-align:center;">
                <div style="display:inline-block;padding:16px 22px;border-radius:16px;background-color:#fff1ec;color:#fd4604;font-size:32px;line-height:1;font-weight:800;letter-spacing:10px;">
                  ${safeCode}
                </div>
              </td>
            </tr>
            <tr>
              <td style="padding:18px 28px 30px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#f6f7f8;border-radius:16px;">
                  <tr>
                    <td style="padding:14px 16px;font-size:13px;line-height:1.5;color:#222222;">
                      Если вы не запрашивали вход, просто проигнорируйте это письмо. Никому не передавайте код.
                    </td>
                  </tr>
                </table>
              </td>
